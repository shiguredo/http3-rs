...
impl Connection {
    ...
    pub fn write_stream_with_flags(
        ...
        let mut flags = flags;
        if fin {
            flags |= shiguredo_ngtcp2_sys::NGTCP2_WRITE_STREAM_FLAG_FIN;
        }
        ...
        if rv == shiguredo_ngtcp2_sys::NGTCP2_ERR_WRITE_MORE as ngtcp2_ssize {
        ...
        if rv == shiguredo_ngtcp2_sys::NGTCP2_ERR_STREAM_DATA_BLOCKED as ngtcp2_ssize {
            return Err(Error::StreamDataBlocked(stream_id));
        }

        if rv == shiguredo_ngtcp2_sys::NGTCP2_ERR_STREAM_SHUT_WR as ngtcp2_ssize {
