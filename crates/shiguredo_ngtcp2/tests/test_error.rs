...
fn test_classify_drop_conn() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_DROP_CONN);
    assert_eq!(
...
fn test_classify_idle_close() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_IDLE_CLOSE);
    assert_eq!(
        err.classify_connection_error(),
        ConnectionErrorKind::SilentDrop
    );
...
fn test_classify_retry() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_RETRY);
    assert_eq!(
        err.classify_connection_error(),
        ConnectionErrorKind::SilentDrop
    );
...
fn test_classify_draining() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_DRAINING);
    assert_eq!(
        err.classify_connection_error(),
...
fn test_classify_closing() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_CLOSING);
    assert_eq!(
        err.classify_connection_error(),
        ConnectionErrorKind::Terminal
    );
...
fn test_classify_discard_pkt() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_DISCARD_PKT);
...
fn test_classify_crypto() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_CRYPTO);
    assert_eq!(
        err.classify_connection_error(),
...
fn test_classify_other_ngtcp2_error() {
    let err = Error::from_ngtcp2(shiguredo_ngtcp2_sys::NGTCP2_ERR_NOBUF);
    assert_eq!(
        err.classify_connection_error(),
        ConnectionErrorKind::TransportClose
    );
