...
pub(crate) fn parse_new_connection_packet(data: &[u8]) -> Option<NewConnectionInfo> {
    if data.len() < shiguredo_ngtcp2_sys::NGTCP2_MAX_UDP_PAYLOAD_SIZE as usize {
        return None;
    }
    if data.len() < 6 {
        return None;
    ...
    if dcid_len < shiguredo_ngtcp2_sys::NGTCP2_MIN_INITIAL_DCIDLEN as usize {
        return None;
    }
...
pub(crate) async fn send_connection_close(
    ...
    match result {
        ...
        Err(e) => {
            eprintln!(
                "[shiguredo_ngtcp2_tokio] failed to write CONNECTION_CLOSE: {}",
                e
            );
            false
        }
