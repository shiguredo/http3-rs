...
use shiguredo_ngtcp2_sys::ngtcp2_transport_params;
...
impl Server {
            ...
            tokio::select! {
                ...
                result = self.socket.recv_from(&mut self.recv_buf) => {
                    match result {
                        ...
                        Err(e) => {
                            eprintln!("[shiguredo_ngtcp2_tokio server] recv error: {}", e);
                            continue;
                        }
                    }
                }
    ...
    async fn handle_new_connection(&mut self, data: &[u8], from: SocketAddr) {
        ...
        let server_scid = match ConnectionId::random(SERVER_SCID_LEN) {
            Some(cid) => cid,
            None => {
                eprintln!("[shiguredo_ngtcp2_tokio server] failed to generate scid");
                return;
            }
        };
        ...
        let tls_session = match self.tls_ctx.create_session() {
            ...
            Err(e) => {
                eprintln!(
                    "[shiguredo_ngtcp2_tokio server] failed to create TLS session: {}",
                    e
                );
                return;
            }
        ...
        let mut conn = {
            ...
            match Connection::server_new(
                ...
                Err(e) => {
                    eprintln!(
                        "[shiguredo_ngtcp2_tokio server] failed to create connection: {}",
                        e
                    );
                    return;
                }
        ...
        let h3_conn = match Http3Connection::server_new(&self.h3_settings) {
            ...
            Err(e) => {
                eprintln!(
                    "[shiguredo_ngtcp2_tokio server] failed to create HTTP/3 connection: {}",
                    e
                );
                return;
            }
    ...
    async fn handle_conn_error(&mut self, conn_key: &ConnectionId, err: Error) {
        match err.classify_connection_error() {
            ...
            ConnectionErrorKind::TransportClose | ConnectionErrorKind::ApplicationClose => {
                eprintln!(
                    "[shiguredo_ngtcp2_tokio server] closing connection: {}",
                    err
                );
            ...
            ConnectionErrorKind::Internal => {
                ...
                // 接続を破棄してサーバーは継続する
                eprintln!("[shiguredo_ngtcp2_tokio server] connection error: {}", err);
                self.remove_connection(conn_key);
            }
        }
    }
            ...
            while let Some(conn) = self.connections.get_mut(&conn_key) {
                match conn.conn.write_pkt(&mut self.send_buf, ts) {
                    ...
                    Ok((written, _)) => {
                        ...
                        {
                            eprintln!("[shiguredo_ngtcp2_tokio server] send error: {}", e);
                            self.remove_connection(&conn_key);
                            break;
                        }
                    }
