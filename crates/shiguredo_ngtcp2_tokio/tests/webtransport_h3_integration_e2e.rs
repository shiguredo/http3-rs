//! shiguredo_ngtcp2_tokio <-> shiguredo_http3 WebTransport 統合 E2E テスト
//!
//! shiguredo_http3 (Sans I/O) の WebTransport プロトコルロジック
//! (Datagram, Capsule, StreamHeader, Session) を shiguredo_ngtcp2_tokio の
...
use shiguredo_ngtcp2_tokio::{ClientWebTransportSession, ServerWebTransportSession};
...
/// shiguredo_http3::Datagram エンコード/デコード ↔ shiguredo_ngtcp2_tokio DATAGRAM 統合テスト
///
/// shiguredo_http3 の Datagram::encode() でエンコードしたバイト列を
/// shiguredo_ngtcp2_tokio の QUIC DATAGRAM として送信し、受信側で Datagram::decode() で
/// デコードして元のデータと一致することを検証する。
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_datagram_encode_decode_over_shiguredo_ngtcp2_tokio() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        let _ = timeout(Duration::from_secs(10), async {
            loop {
                ...
                // ここでは shiguredo_ngtcp2_tokio レベルで受信した DATAGRAM を取得
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_datagram_roundtrip_over_shiguredo_ngtcp2_tokio() {
    let (cert_path, key_path) = generate_test_certs();
...
/// shiguredo_http3 Session と shiguredo_ngtcp2_tokio WebTransport の統合テスト
///
/// shiguredo_http3 の Session 構造体を使ってセッション状態を管理しながら、
/// shiguredo_ngtcp2_tokio で実際のデータ送受信を行う。Session::add_stream / get_stream / stream_count
/// でストリーム管理が shiguredo_ngtcp2_tokio のストリーム操作と整合することを検証する。
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_session_state_with_shiguredo_ngtcp2_tokio_streams() {
    ...
    // クライアント: shiguredo_http3 Session でストリーム管理しながら shiguredo_ngtcp2_tokio で送受信
    let client_result = timeout(Duration::from_secs(10), async {
        let mut session =
            ClientWebTransportSession::connect_insecure(server_addr, "localhost", "/webtransport")
                .await
        ...
        // shiguredo_ngtcp2_tokio でストリームを開き、shiguredo_http3 Session にも登録
        let stream_count = 3;
        let mut stream_ids = Vec::new();

        for i in 0..stream_count {
            let shiguredo_ngtcp2_tokio_stream_id =
                session.open_bidi_stream().expect("ストリーム作成失敗");
            stream_ids.push(shiguredo_ngtcp2_tokio_stream_id);

            // shiguredo_http3 Session にストリームを追加
            let h3_stream = Stream::new(
                shiguredo_ngtcp2_tokio_stream_id as u64,
                session_id as u64,
                true,
            );
            ...
            assert!(
                h3_session
                    .get_stream(shiguredo_ngtcp2_tokio_stream_id as u64)
                    .is_some()
            );

            // shiguredo_ngtcp2_tokio でデータ送信
            let data = format!("Stream {} data", i);
            session
                .send_stream_data(shiguredo_ngtcp2_tokio_stream_id, data.as_bytes(), true)
                .await
                .expect("データ送信失敗");
        }
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_stream_header_bidi_over_shiguredo_ngtcp2_tokio() {
...
/// でデコードして session_id が実セッション ID と一致することを検証する。
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_stream_header_uni_over_shiguredo_ngtcp2_tokio() {
    use shiguredo_http3::webtransport::StreamHeader;
...
///
/// shiguredo_ngtcp2_tokio で実際にセッション確立後、shiguredo_http3 の ConnectRequest::new() / validate()
/// と ConnectResponse::new(200).is_success() が実セッションパラメータで正しく動作することを検証する。
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_connect_validation_with_shiguredo_ngtcp2_tokio_session() {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_error_code_conversion_with_shiguredo_ngtcp2_tokio() {
...
///
/// shiguredo_ngtcp2_tokio セッションの各段階で shiguredo_http3 Session の状態を同期管理:
/// Pending -> Connecting -> Established -> ストリーム追加/削除 -> drain() -> Closed。
/// 実ネットワーク操作と状態遷移が整合することを検証する。
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_session_lifecycle_with_shiguredo_ngtcp2_tokio() {
...
/// 実際の送受信量と一致することを検証する。
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_stream_byte_tracking_with_shiguredo_ngtcp2_tokio() {
    use shiguredo_http3::webtransport::{Session, Stream};
    ...
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        for payload in &payloads {
            ...
            session
                .send_stream_data(shiguredo_ngtcp2_tokio_stream_id, payload, true)
                .await
                .expect("データ送信失敗");

            stream_tracking.push((shiguredo_ngtcp2_tokio_stream_id, payload.len()));
        }
        ...
        // 各ストリームの送受信バイト数を検証
        for (shiguredo_ngtcp2_tokio_stream_id, expected_len) in &stream_tracking {
            ...
            assert_eq!(
                ...
                *expected_len as u64,
                ...
                shiguredo_ngtcp2_tokio_stream_id
            );

            // エコーバックされたデータがあれば受信バイト数も検証
            if let Some(recv_data) = received_per_stream.get(shiguredo_ngtcp2_tokio_stream_id) {
                assert_eq!(
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_all_capsule_types_over_shiguredo_ngtcp2_tokio() {
    let (cert_path, key_path) = generate_test_certs();
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_h3_multiple_datagrams_roundtrip_over_shiguredo_ngtcp2_tokio() {
    let (cert_path, key_path) = generate_test_certs();
