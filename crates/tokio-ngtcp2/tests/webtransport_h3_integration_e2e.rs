...
//! (Datagram, Capsule, StreamHeader) を ngtcp2 の
//! 実ネットワーク I/O と組み合わせて検証する。
//!
//! ngtcp2 経路の HTTP/3 / WebTransport は nghttp3 が処理するため
//! `shiguredo_http3::Connection` はこの経路に関与しない。`Connection` 側の
//! フロー制御は `tests/test_webtransport_flow_control.rs` で検証する。
