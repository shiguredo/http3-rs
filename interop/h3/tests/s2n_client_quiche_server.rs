//! s2n-quic クライアント ↔ quiche サーバー 相互運用性テスト
...
use interop_h3::{generate_shared_certificate, save_certificate_files, start_quiche_server};
...
async fn test_http3_request_response() {
    ...
    match client_result {
        Ok(Ok(response)) => {
            ...
            assert_eq!(
                response_str.trim(),
                "Hello from quiche HTTP/3 server!",
                "レスポンスボディが期待値と一致しない: {:?}",
                response_str
            );
        }
