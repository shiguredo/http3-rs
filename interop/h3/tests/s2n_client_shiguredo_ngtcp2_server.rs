//! s2n-quic クライアント ↔ shiguredo_ngtcp2 サーバー 相互運用性テスト
//!
//! s2n-quic クライアント (shiguredo_http3) から shiguredo_ngtcp2 サーバー (nghttp3) への
...
use shiguredo_ngtcp2_tokio::Server;
...
async fn start_shiguredo_ngtcp2_server(
    ...
    eprintln!("[shiguredo_ngtcp2 server] サーバー起動: {}", local_addr);

    Ok((server, local_addr))
}

/// shiguredo_ngtcp2 サーバーを実行
async fn run_shiguredo_ngtcp2_server(
    mut server: Server,
) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    eprintln!("[shiguredo_ngtcp2 server] サーバー実行開始");

    let result = tokio::time::timeout(
        Duration::from_secs(10),
        server.run(|addr, event| {
            eprintln!(
                "[shiguredo_ngtcp2 server] イベント: {:?} from {:?}",
                event, addr
            );
            ...
            match event {
                Http3Event::HeadersEnd { stream_id, .. } => {
                    eprintln!(
                        "[shiguredo_ngtcp2 server] ヘッダー終了: stream_id = {}",
                        stream_id
                    );

                    let response_headers = vec![
                        ShiguredoNgtcp2Header::status(200),
                        ShiguredoNgtcp2Header::new(b"content-type", b"text/plain; charset=utf-8"),
                    ];
    ...
    match result {
        Ok(Ok(())) => eprintln!("[shiguredo_ngtcp2 server] サーバー正常終了"),
        Ok(Err(e)) => eprintln!("[shiguredo_ngtcp2 server] サーバーエラー: {:?}", e),
        Err(_) => eprintln!("[shiguredo_ngtcp2 server] タイムアウト"),
    }
...
async fn test_http3_request_response() {
    ...
    // shiguredo_ngtcp2 サーバーを起動
    let (server, server_addr) = start_shiguredo_ngtcp2_server(&cert_path, &key_path)
        .await
    ...
    let server_task = tokio::spawn(async move {
        if let Err(e) = run_shiguredo_ngtcp2_server(server).await {
...
async fn test_post_request_with_body() {
    ...
    let (mut server, server_addr) = start_shiguredo_ngtcp2_server(&cert_path, &key_path)
        .await
        .expect("test must succeed");
    let port = server_addr.port();
    ...
    let server_task = tokio::spawn(async move {
        let _ = tokio::time::timeout(
            ...
            server.run(|_addr, event| match event {
                Http3Event::HeadersEnd { stream_id, .. } => {
                    eprintln!(
                        "[shiguredo_ngtcp2 server] POST 受信: stream_id = {}",
                        stream_id
                    );
                    Some((
                        vec![
                            ShiguredoNgtcp2Header::status(200),
                            ShiguredoNgtcp2Header::new(
                                b"content-type",
...
///
/// shiguredo_ngtcp2 サーバーがカスタムヘッダーを含むレスポンスを返し、
                ...
                Http3Event::HeadersEnd { stream_id, .. } => {
                    ...
                    Some((
                        vec![
                            ...
                            ShiguredoNgtcp2Header::new(b"x-server", b"nghttp3"),
                            ShiguredoNgtcp2Header::new(b"x-custom-header", b"custom-value"),
                        ],
                        b"Hello".to_vec(),
                    ))
                }
...
///
/// shiguredo_ngtcp2 サーバーがパスに応じて 404 を返し、
        ...
        let _ = tokio::time::timeout(
            ...
            server.run(move |_addr, event| match event {
                ...
                Http3Event::HeadersEnd { stream_id, .. } => {
                    ...
                    eprintln!(
                        "[shiguredo_ngtcp2 server] リクエストパス: {}",
                        String::from_utf8_lossy(&path)
                    );
                    if path == b"/not-found" {
                        Some((vec![ShiguredoNgtcp2Header::status(404)], vec![]))
                    } else {
                        Some((vec![ShiguredoNgtcp2Header::status(200)], b"OK".to_vec()))
                    }
                }
                _ => None,
            }),
