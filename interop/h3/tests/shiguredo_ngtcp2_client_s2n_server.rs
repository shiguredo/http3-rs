//! shiguredo_ngtcp2 クライアント ↔ s2n-quic サーバー 相互運用性テスト
//!
//! shiguredo_ngtcp2 クライアント (nghttp3) から s2n-quic サーバー (shiguredo_http3) への
...
/// shiguredo_ngtcp2 クライアントのレスポンス
struct ShiguredoNgtcp2Response {
    /// ステータスコード
    status: u16,
    /// レスポンスヘッダー
...
/// shiguredo_ngtcp2 クライアントでリクエストを送信してレスポンス全体を返す
async fn send_shiguredo_ngtcp2_request(
    server_addr: SocketAddr,
    method: &str,
    path: &str,
    body: Vec<u8>,
) -> Result<ShiguredoNgtcp2Response, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let headers = vec![
        ShiguredoNgtcp2Header::method(method),
        ShiguredoNgtcp2Header::path(path),
            ...
            match event {
                ...
                Http3Event::StreamEnd { .. } => {
                    let status = response_headers
                        ...
                        .unwrap_or(0);
                    return Ok(ShiguredoNgtcp2Response {
                        status,
...
///
/// shiguredo_ngtcp2 クライアントがボディ付き POST リクエストを送信できること、
...
async fn test_post_request_with_body() {
    ...
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        send_shiguredo_ngtcp2_request(server_addr, "POST", "/", b"Hello, server!".to_vec()),
    )
    .await;
...
/// shiguredo_ngtcp2 クライアントが正しく受信できることを確認する。
#[tokio::test]
async fn test_response_custom_headers() {
    ...
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        send_shiguredo_ngtcp2_request(server_addr, "GET", "/", vec![]),
    )
    .await;
...
/// shiguredo_ngtcp2 クライアントが正しくステータスコードを受信できることを確認する。
#[tokio::test]
async fn test_status_404() {
    ...
    let result = tokio::time::timeout(
        Duration::from_secs(10),
        send_shiguredo_ngtcp2_request(server_addr, "GET", "/not-found", vec![]),
    )
    .await;
