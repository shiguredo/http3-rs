...
//! tquic クライアント ↔ shiguredo_ngtcp2 サーバー 相互運用性テスト
...
async fn start_shiguredo_ngtcp2_server(
    ...
    eprintln!("[shiguredo_ngtcp2 server] server started: {}", local_addr);
    ...
    let server_task = tokio::spawn(async move {
        ...
        let _ = tokio::time::timeout(
            ...
            server.run(|addr, event| {
                eprintln!(
                    "[shiguredo_ngtcp2 server] event: {:?} from {:?}",
                    event, addr
                );
                ...
                match event {
                    Http3Event::HeadersEnd { stream_id, .. } => {
                        eprintln!(
                            "[shiguredo_ngtcp2 server] headers end: stream_id = {}",
                            stream_id
                        );
