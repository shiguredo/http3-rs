//! quinn (h3-webtransport) クライアント ↔ shiguredo_ngtcp2 サーバー WebTransport 相互運用性テスト
//!
            ...
            server.run(|addr, session_id, event| {
                match &event {
                    Http3Event::HeadersEnd { stream_id, .. } => {
                        eprintln!(
                            "[shiguredo_ngtcp2 server] CONNECT: addr={} session={} stream={}",
                    ...
                    _ => {
                        eprintln!(
                            "[shiguredo_ngtcp2 server] event: {:?} session={}",
                            event, session_id
                        );
                    }
                }
