//! s2n-quic クライアント + shiguredo_ngtcp2 サーバー WebTransport 相互運用性テスト
//!
...
use shiguredo_ngtcp2_tokio::ServerWebTransportSession;
...
async fn test_webtransport_session() {
    ...
    eprintln!("[shiguredo_ngtcp2 server] started: {}", server_addr);
    ...
    let server_task = tokio::spawn(async move {
            ...
            server.run(|addr, session_id, event| {
                match &event {
                    Http3Event::HeadersEnd { stream_id, .. } => {
                        eprintln!(
                            "[shiguredo_ngtcp2 server] CONNECT request: addr={} session_id={} stream_id={}",
                    ...
                    Http3Event::WebTransportData {
                        ...
                        eprintln!(
                            "[shiguredo_ngtcp2 server] WebTransport data: session_id={} stream_id={} data={}",
                            session_id,
                            stream_id,
        ...
        match result {
            Ok(r) => r,
            Err(_) => {
                eprintln!("[shiguredo_ngtcp2 server] timed out");
                Ok(())
            }
        }
    });
    ...
    match client_result {
        Ok(Ok(session)) => {
            eprintln!(
                "test passed: s2n client + shiguredo_ngtcp2 server session_id={}",
                session.session_id()
            );
        }
        Ok(Err(e)) => {
...
///
/// s2n-quic クライアントから shiguredo_ngtcp2 サーバーへの単方向ストリーム送信をテストする。
...
/// shiguredo_ngtcp2 サーバーが WebTransportData イベントでデータを受信することを確認する。
#[serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
/// shiguredo_ngtcp2 サーバーが双方向ストリームを開いてデータを送信するテスト
/// (RFC draft-ietf-webtrans-http3-15 Section 4.3)
///
/// recv_once / open_bidi_stream_for / send_stream_data_for API を使用して、
/// shiguredo_ngtcp2 サーバーが nghttp3 経由でデータを送信する。
...
async fn test_server_opens_bidi_stream() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        while client_addr.is_none() {
            ...
            if tokio::time::Instant::now() > deadline {
                panic!("[shiguredo_ngtcp2 server] session establishment timed out");
            }
        }

        let addr = client_addr.expect("test must succeed");
        eprintln!(
            "[shiguredo_ngtcp2 server] session established: addr={}",
            addr
        );
        ...
        eprintln!(
            "[shiguredo_ngtcp2 server] stream opened: stream_id={}",
            stream_id
        );

        server
            .send_stream_data_for(
                &addr,
                stream_id,
                b"Hello from shiguredo_ngtcp2 server!",
                true,
            )
            .expect("data send enqueue failed");
        server.flush().await.expect("flush failed");
        eprintln!("[shiguredo_ngtcp2 server] data sent");
    ...
    match client_result {
        Ok(Ok(data)) => {
            assert_eq!(
                data, b"Hello from shiguredo_ngtcp2 server!",
                "received data must match"
            );
            eprintln!(
                "test passed: shiguredo_ngtcp2 server bidi stream data={}",
                String::from_utf8_lossy(&data)
            );
        }
        Ok(Err(e)) => panic!("client error: {:?}", e),
...
/// shiguredo_ngtcp2 サーバーが同じストリームでエコー返信することを確認する。
#[serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn test_bidi_stream_echo() {
    ...
    let server_task = tokio::spawn(async move {
        ...
        while received.is_empty() {
            ...
            server
                ...
                .ok();
            if tokio::time::Instant::now() > deadline {
                panic!("[shiguredo_ngtcp2 server] data receive timed out");
            }
        }
    ...
    let client_result = timeout(Duration::from_secs(15), async {
        ...
        // エコーを受信 (shiguredo_ngtcp2 サーバーは WT ヘッダーなしで返信)
...
/// shiguredo_ngtcp2 サーバーが全ストリームのデータを正しく受信することを確認する。
#[serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
/// shiguredo_ngtcp2 サーバーが全データを正確に受信することを確認する。
#[serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
///
/// s2n-quic クライアントから shiguredo_ngtcp2 サーバーへの Datagram 送受信をテストする。
