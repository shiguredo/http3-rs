//! shiguredo_ngtcp2 クライアント ↔ quinn (h3-webtransport) サーバー WebTransport 相互運用性テスト
//!
//! h3-webtransport は draft-02 を使用。
//! shiguredo_ngtcp2 (nghttp3) は draft-15 を使用するためネゴシエーション失敗が想定される。
...
async fn test_webtransport_session() {
    ...
    // shiguredo_ngtcp2 クライアントで WT セッション確立を試行
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        match session_result {
            Ok(session_id) => {
                eprintln!(
                    "[shiguredo_ngtcp2 client] WT session established: session_id = {}",
                    session_id
                );
                Some(session_id)
            }
            Err(e) => {
                eprintln!("[shiguredo_ngtcp2 client] WT session failed: {:?}", e);
                None
            }
        }
    })
