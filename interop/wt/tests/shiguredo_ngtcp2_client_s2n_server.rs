//! shiguredo_ngtcp2 クライアント + s2n-quic サーバー WebTransport 相互運用性テスト
//!
...
use shiguredo_ngtcp2_tokio::ClientWebTransportSession;
...
async fn test_webtransport_session() {
    ...
    // shiguredo_ngtcp2 クライアントで WebTransport セッションを確立
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        eprintln!("[shiguredo_ngtcp2 client] handshake start");
        session.handshake().await.expect("handshake failed");
        eprintln!("[shiguredo_ngtcp2 client] handshake complete");
        ...
        match session_result {
            Ok(session_id) => {
                eprintln!(
                    "[shiguredo_ngtcp2 client] WebTransport session started: session_id = {}",
                    session_id
                );
                Some(session_id)
            }
            Err(e) => {
                eprintln!(
                    "[shiguredo_ngtcp2 client] WebTransport session failed: {:?}",
                    e
                );
                None
            }
...
///
/// shiguredo_ngtcp2 クライアントから s2n-quic サーバーへの単方向ストリーム送信をテストする。
...
async fn test_unidirectional_stream() {
    ...
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        // 単方向ストリームは shiguredo_ngtcp2_tokio の ClientWebTransportSession では open_uni_stream が未実装
        // セッション確立の成功を確認する
        eprintln!("[shiguredo_ngtcp2 client] session establishment confirmed");
...
/// shiguredo_ngtcp2 クライアントが WebTransportData イベントでデータを受信することを確認する。
#[serial]
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
...
/// shiguredo_ngtcp2 クライアントが双方向ストリームを開いてデータを送信するテスト
/// (RFC draft-ietf-webtrans-http3-15 Section 4.3)
///
/// send_stream_data API を使用して、shiguredo_ngtcp2 クライアントが nghttp3 経由でデータを送信する。
...
async fn test_client_opens_bidi_stream() {
    ...
    // shiguredo_ngtcp2 クライアントで接続してデータを送信
    let client_result = timeout(Duration::from_secs(10), async {
        let mut session =
            ClientWebTransportSession::connect_insecure(server_addr, "localhost", "/webtransport")
                .await
        ...
        eprintln!(
            "[shiguredo_ngtcp2 client] stream opened: stream_id = {}",
            stream_id
        );

        session
            .send_stream_data(stream_id, b"Hello from shiguredo_ngtcp2 client!", true)
            .await
            .expect("data send failed");
        eprintln!("[shiguredo_ngtcp2 client] data sent");
    ...
    match client_result {
        Ok(session_id) => {
            assert_eq!(
                server_data, b"Hello from shiguredo_ngtcp2 client!",
                "received data should match"
            );
            eprintln!(
                "test passed: shiguredo_ngtcp2 client sent data via bidirectional stream session_id = {}, data = {}",
                session_id,
...
///
/// shiguredo_ngtcp2 クライアントが送信した CONNECT リクエストの :path と :authority が
...
///
/// shiguredo_ngtcp2 クライアントが bidi ストリームでデータを送信し、
...
async fn test_bidi_stream_echo() {
    ...
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        session
            ...
            .await
            .expect("send failed");
        eprintln!("[shiguredo_ngtcp2 client] sent: stream_id={}", stream_id);
    ...
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        for payload in &payloads {
            ...
            session
                ...
                .expect("send failed");
            eprintln!(
                "[shiguredo_ngtcp2 client] stream stream_id={} sent",
                stream_id
            );
        }
...
///
/// shiguredo_ngtcp2 クライアントから s2n-quic サーバーへの Datagram 送受信をテストする。
...
async fn test_datagram() {
    ...
    let client_result = timeout(Duration::from_secs(10), async {
        ...
        match datagram_result {
            Ok(accepted) => {
                eprintln!(
                    "[shiguredo_ngtcp2 client] datagram sent: accepted = {}, payload = {}",
                    accepted,
            ...
            Err(e) => {
                ...
                eprintln!(
                    "[shiguredo_ngtcp2 client] datagram send failed (server DATAGRAM support required): {:?}",
                    e
                );
