...
mod tests {
    ...
    #[test]
    fn test_client_connection_wt_flow_control_capsules() {
        ...
        assert!(
            client.wt_session_flow_control_enabled(0),
            "フロー制御が有効になっていること"
        );
        assert!(
            !client.take_wt_pending_capsules(0).is_empty(),
            "初期フロー制御カプセルが取得できること"
        );
        assert!(
            client.take_wt_pending_capsules(0).is_empty(),
